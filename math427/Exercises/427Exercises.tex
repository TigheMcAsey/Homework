...
\begin{equation}
    ...
    \emph{exercise - }\label{KEx3} Recall in class we used the state sum formula/hypercube to compute the cochain complex \(\mathcal{C}\left(\KPL\!\RIIcn[scale = 0.6]\!\KPR\right)\), the choices made in this construction where such that the Euler characteristic of the complex was made to be \(\gen{D}\). When recording the hypercube we record the resulting fully simplified knot states with a "height" corresponding to the number of left/right simplifications (this corresponds to the factor of \(q\) in \href{KEx1}{the first exercise of this section}). This is compatible with the
    ...
    We have discussed the hypercube as well as its height grading thus far, however to construct a cochain complex we need a module or vectorspace. In the hypercube each of our knots is reduced to some \(\bigsqcup\bigcirc\), where in the jones poly
    
    Let \(D := \KPL\!\KPB\!\KPR\), compute \(\mathcal{C}Kh(\vec{D})\).
