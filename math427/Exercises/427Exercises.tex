...
\begin{equation}
    ...
    \begin{equation*}
        ...
        \end{tikzcd}
    \end{equation*}
    is a short exact sequence of chain complexes, then we get the short exact sequence on homology (\textcolor{red}{TODO : Prove this}):
    \begin{equation*}
        \begin{tikzcd}
    ...
    \emph{exercise - } Write the diagram for the following knot in \(\mathcal{C}_X(\text{Mat}(\text{Cob}/\ell(B)))\). Compute an equivalent diagram in \(\mathcal{C}_X(k[H])\)
    ...
    \emph{exercise - } Compute \(G\) in the following delooping on \(\text{Cob}/\ell(4)\)
